...
void mainMenu(Graph& g,
    ...
    while (true) {
        ...
        case 2:
            runScenarios(g, hospitals, ambulances, requests, log);
        ...
        case 3: {
            ...
            cout << "  Location node: "; cin >> node; cin.ignore();
            cout << "  Priority (1=RED 2=ORANGE 3=YELLOW 4=GREEN): "; cin >> pri; cin.ignore();
            pri = max(1, min(4, pri));
        ...
        case 4: {
            ...
            int hid;
            cout << "  Hospital ID: "; cin >> hid; cin.ignore();
            engine.releaseBed(hid);
            break;
        }
        case 5: {
            printSubHeader("BLOCK ROAD");
            cout << "  Enter two node IDs to block road between them\n";
            int u, v;
            cout << "  Node A: "; cin >> u; cin.ignore();
            cout << "  Node B: "; cin >> v; cin.ignore();
            g.blockRoad(u, v);
        ...
        case 6:
            if (requests.empty()) cout << "  No requests yet.\n";
        ...
        case 8:
            ...
            break;
        case 9:
            displayHospitalStaffView(hospitals);
            break;
        case 10:
            displayDispatchLog(log, 30);
            break;
        case 11: {
            int src, dst;
            cout << "  From node: "; cin >> src; cin.ignore();
            cout << "  To node:   "; cin >> dst; cin.ignore();
        ...
        case 12: {
            int node, pri;
            cout << "  From node: "; cin >> node; cin.ignore();
            cout << "  Priority (1=RED 2=ORANGE 3=YELLOW 4=GREEN): ";
            cin >> pri; cin.ignore();
        ...
        case 13: {
            printSubHeader("NETWORK LOAD OVERVIEW");
            int totalBeds = 0, usedBeds = 0;
            for (auto& h : hospitals) {
                totalBeds += h.totalBeds;
                usedBeds  += (h.totalBeds - h.availableBeds);
            }
            cout << "  System-wide bed utilisation: "
                 << BOLD << usedBeds << "/" << totalBeds << " ("
                 << (int)(100.0*usedBeds/totalBeds) << "%)" << RESET << "\n\n";

            int ambAvail = 0;
            for (auto& a : ambulances) if (a.available) ambAvail++;
            cout << "  Ambulances available: "
                 << COL_GREEN << ambAvail << RESET << "/" << ambulances.size() << "\n\n";

            cout << "  Pending requests: ";
            int pending = 0;
            for (auto& r : requests) if (!r.resolved) pending++;
            if (pending > 0) cout << COL_RED << pending << RESET;
            else cout << COL_GREEN << "0" << RESET;
            cout << "\n";
